...
@HandlerTrigger(msgId = MsgId.EN_MessageId.E_ROOM_ENTER_PLAYER_VALUE)
public class RoomEnterPlayerHandler extends OuterClientHandle {
    @Override
    public void clientExecute(ChannelHandlerContext ctx, short code, byte[] bodyBytes, int reqSeq) {
        if(code != Code.En_Code.E_SUSS_VALUE){
            log.info("receive E_ROOM_ENTER_PLAYER code:{}", code);
            return;
        }
    ...
    public void sendTestData(ChannelHandlerContext ctx) {
        super.sendMessage(ctx, MsgId.EN_MessageId.E_ROOM_ENTER_PLAYER, null, ClientConstant.atomicInteger.getAndIncrement(), Code.En_Code.E_SUSS);
    }
}